    ...
    for i in range(max_loop):
        ...
        if len(hull.vertices) <= 10:
            ...
            if option == "all_pixel_colors":  ### basic one.
                reconstruction_errors = outsidehull_points_distance(hull.points[hull.vertices].clip(0.0, 1.0),
                                                                    data.reshape((-1, 3)))

            elif option == "unique_pixel_colors":  ### results should be same with above opiton, but faster
                reconstruction_errors = outsidehull_points_distance_unique_data_version(
                ...
                reconstruction_errors = outsidehull_points_distance_for_using_origin_hull_vertices(
                    hull.points[hull.vertices].clip(0.0, 1.0), data.reshape((-1, 3)),
                    origin_vertices.reshape((-1, 3)))  ### may use 5/255.0 to be threshold.
...
def recover_ASAP_weights_using_scipy_delaunay(Hull_vertices, data, option=1):
    # ##modified from https://codereview.stackexchange.com/questions/41024/faster-computation-of-barycentric
    # -coordinates-for-many-points (Gareth Rees) Load points
...
def Get_ASAP_weights_using_Tan_2016_triangulation_and_then_barycentric_coordinates(
    ...
    for face_vertex_ind in hull.simplices:
        if (face_vertex_ind != 0).all():
            ...
            try:
                # ### use try here, because sometimes the tetra is nearly flat, will cause qhull error to stop,
                # we do not want to stop, we just skip. print (tetra)
    ...
    temp = (origin_order_mixing_weights.reshape((img_shape[0], img_shape[1], -1, 1)) * origin_order_tetra_prime.reshape(
        (1, 1, -1, 3))).sum(axis=2)
