...
class Trainer:
    ...
    def render_test(self, args):
        ...
        if args.render_train:
            ...
            PSNRs_test = evaluation(train_dataset, tensorf, args, self.renderer, f'{logfolder}/imgs_train_all/',
                                    N_vis=-1, N_samples=-1, white_bg=white_bg, ndc_ray=ndc_ray, device=self.device)
            print(f'======> {args.expname} train all psnr: {np.mean(PSNRs_test)} <========================')

        if args.render_test:
            os.makedirs(f'{logfolder}/{args.expname}/imgs_test_all', exist_ok=True)
            evaluation(test_dataset, tensorf, args, self.renderer, f'{logfolder}/{args.expname}/imgs_test_all/',
                       N_vis=-1, N_samples=-1, white_bg=white_bg, ndc_ray=ndc_ray, device=self.device)
    ...
    def reconstruction(self, args):
        ...
        for iteration in pbar:
            ...
            rays_train, rgb_train = allrays[ray_idx], allrgbs[ray_idx].to(self.device)
            ...
            if iteration % args.vis_every == args.vis_every - 1 and args.N_vis != 0:
                PSNRs_test = evaluation(test_dataset, tensorf, args, self.renderer, f'{logfolder}/imgs_vis/',
                                        N_vis=args.N_vis,
        ...
        if args.render_test:
            ...
            PSNRs_test = evaluation(test_dataset, tensorf, args, self.renderer, f'{logfolder}/imgs_test_all/',
                                    N_vis=-1, N_samples=-1, white_bg=white_bg, ndc_ray=ndc_ray, device=self.device)
        ...
        if args.render_path:
            ...
            evaluation_path(test_dataset, tensorf, c2ws, self.renderer, f'{logfolder}/imgs_path_all/',
                            N_vis=-1, N_samples=-1, white_bg=white_bg, ndc_ray=ndc_ray, device=self.device)
...
def main(args):
    ...
    trainer = Trainer()
    if args.export_mesh:
        trainer.export_mesh(args)

    if args.render_only and (args.render_test or args.render_path):
        trainer.render_test(args)
    else:
        trainer.reconstruction(args)
