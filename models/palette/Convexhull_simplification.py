# ####directly copy from SILD_convexhull_simplification-minimize_adding_volume_or_normalized_adding_volume.ipynb
# 2016.01.11 ### and then remove many unrelated codes.
...
def remove_one_edge_by_finding_smallest_adding_volume_with_test_conditions(mesh, option):
    ...
    for edge_index in range(len(edges)):
        ...
        # ### now find a point, so that for each face in related_faces will create a positive volume tetrahedron
        # using this point. ## minimize c*x. w.r.t. A*x<=b
        c = np.zeros(3)
        A = []
        b = []
        ...
        #         ##### means all normal dot value are positive
        #         if edge_normal_test(vertices,faces,face_index,vertex1,vertex2)==1:
...
def main():
    ...
    for i in range(N):
        ...
        if len(newhull.vertices) <= 10:
            ...
            with open(name, 'w') as myfile:
                json.dump({'vs': newhull.points[newhull.vertices].tolist(),
                           'faces': newhull.points[newhull.simplices].tolist()}, myfile, indent=4)
            ...
            Image.fromarray(pigments_colors).save(
                os.path.splitext(js_output_clip_file)[0] + ('-%02d.png' % len(newhull.vertices)))
    ...
    #     json.dump(
    #         {'vs': newhull.points[newhull.vertices].tolist(), 'faces': newhull.points[newhull.simplices].tolist()},
    #         myfile, indent=4)

    with open(js_output_file_origin, 'w') as myfile_origin:
        json.dump({'vs': origin_hull.points[origin_hull.vertices].tolist(),
                   'faces': origin_hull.points[origin_hull.simplices].tolist()}, myfile_origin, indent=4)
